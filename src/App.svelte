<script>
  ...
  function getCoordinate(e, imageSize, elementRef) {
    ...
    if (e.clientX > imageSize.left + imageSize.width - elementSize.width / 2) {
      ...
      coordinate.x =
        parseFloat(elementRef.style.left) +
        e.clientX -
        (elementSize.left + elementSize.width / 2);
    }
    ...
    if (e.clientY > imageSize.top + imageSize.height - elementSize.height / 2) {
      ...
      coordinate.y =
        parseFloat(elementRef.style.top) +
        e.clientY -
        (elementSize.top + elementSize.height / 2);
    }
    return coordinate;
  }
  ...
  function dragEnd(e) {
    if (current !== null && current !== undefined) {
      ...
      tags[current] = {
        ...tags[current],
        ...{ left: newCoordinate.x, top: newCoordinate.y }
      };
    }
  }
</script>
...
<div class="image" on:click={e => addNewTag(e)}>
  {#each tags as tag, tagIndex}
    <div
      ...
      <div
        class="delete {current === tagIndex ? '' : 'hide'}"
